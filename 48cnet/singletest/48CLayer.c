...
float MultiplyByElement18_48(float m1[][18][18], float m2[][18][18], int size){
    int i,j,k;
    float result = 0.0;
...
float* CaliLayer48(float img[][48], int height, int width, int channels){
    ...
    */
    ...
    float output6[256];
    for (i = 0; i < 256; i++){
        output6[i] = bias3[i] + MultiplyByElement18_48(filter3[i], output3, 18);
        if (output6[i] < 0){
            output6[i] = 0.0;
        }
        // printf("output6[%d] = %f\n", i, output6[i]);
