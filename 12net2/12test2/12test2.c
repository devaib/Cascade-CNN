        ...
        for (row = 0; row + 12 <= height; row += Stride){
            for (col = 0; col + 12 <= width; col += Stride){
                ...
                if (res > 0.5){
                    ...
                    if (cali_x >= WIDTH || cali_y >= HEIGHT || cali_w <= 0 || cali_h <= 0){
                        continue;
                    }
                    cali_x = max(cali_x, 0);
                    cali_y = max(cali_y, 0);
                    ...
                    IplImage *cali12Img = cvCreateImage(cvSize(12, 12), IPL_DEPTH_8U, 1);
                    cvResize(originalImg,cali12Img, CV_INTER_AREA);;

                    IplImage *tempImg = cvCreateImage(cvSize(100, 100), IPL_DEPTH_8U, 1);
                    cvResize(cali12Img, tempImg, CV_INTER_AREA);;
                    cvNamedWindow("12 net2", CV_WINDOW_AUTOSIZE);
                    cvMoveWindow("12 net2", 1150, 100);
                    cvShowImage("12 net2", tempImg);
                    ...
                    for (i = 0; i < 12; i++){
                        for (j = 0; j < 12; j++){
                            img[i][j] = data[i * cali12Img->widthStep+ j * cali12Img->nChannels] / 255.0;
                            mean_x += img[i][j];
                            mean_x2 += pow(img[i][j], 2);
                        }
                    }
                    ...
                    res = firstLayer(img, 12, 12, cali12Img->nChannels);
                    printf("\nres = %f\n", res);
                    /*
                    if (res > 0.5){
