...
for i,channel in ipairs(channels) do
  for j = 1, numlbls do
     for k=1, #testData.data[j] do
     testData.data[j][k][{ i,{},{} }]:add(-mean[i])
     testData.data[j][k][{ i,{},{} }]:div(std[i])
     end
  end
end
...
for i,channel in ipairs(channels) do
  ...
  for j = 1, numlbls do
     for k=1, #testData.data[j] do
     testMean=testMean+testData.data[j][k][{ i,{},{} }]:mean()
     testStd=testStd+testData.data[j][k][{ i,{},{} }]:std()
     cnt=cnt+1
     end
  end
  testMean=testMean/cnt
  testStd=testStd/cnt
